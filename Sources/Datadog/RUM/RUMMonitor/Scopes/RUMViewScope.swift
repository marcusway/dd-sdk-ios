...
internal class RUMViewScope: RUMScope, RUMContextProvider {
    ...
    init(
        ...
        if let cpuReader = RUMFeature.instance?.vitalCPUReader,
           let memoryReader = RUMFeature.instance?.vitalMemoryReader,
           let refreshRateReader = RUMFeature.instance?.vitalRefreshRateReader {
    ...
    private func sendViewUpdateEvent(on command: RUMCommand) {
        ...
        let eventData = RUMViewEvent(
            ...
            view: .init(
                ...
                cpuTicksPerSecond: nil, //cpuInfo?.greatestDiff.flatMap { $0 / Double(timeSpent) },
                crash: nil,
